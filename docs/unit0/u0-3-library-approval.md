...
이 서브유닛에서는 두 가지를 만듭니다. 성격이 다른 두 저장소와, 그 위에 얹는 **사람의 승인 장치**입니다.
...
{: .note }
이력서 원본은 에이전트의 Knowledge에 넣지 않습니다. "기준"이 아니라 "대상 데이터"이기 때문입니다. 필요할 때 `ResumeLink`로 그 한 건만 꺼내 봅니다(Unit 4의 면접 질문 생성). 지금은 "원본은 라이브러리에 산다"만 기억하면 됩니다.

### 콘텐츠 승인 = 사람의 승인 장치

**콘텐츠 승인**은 SP에 내장된 기능입니다. 켜 두면 새로 만들어진 항목이 곧바로 공개되지 않고 **보류 중** 상태로 시작합니다. 승인자가 검토해 **승인됨**으로 바꿔야 비로소 일반 조회에 나타납니다.
...
{: .note }
콘텐츠 승인이 켜진 목록은 항목을 **수정할 때마다** 승인 상태가 다시 **보류 중**으로 리셋됩니다. 이 동작은 지침으로 막을 수 없는 SP의 서버 부작용으로, Unit 5에서 "왜 상태 변경은 흐름이어야 하는가"의 핵심 근거가 됩니다. 지금은 "수정하면 다시 승인이 필요해진다"만 기억하세요.
...
환경 셋업을 모두 마쳤습니다 🎉 — 사이트, 지원자 마스터 목록과 컬럼, 이력서 보관함, 그리고 콘텐츠 승인까지. 이제 이 집에 데이터를 자동으로 채워 넣는 **적재 흐름**을 만듭니다.
