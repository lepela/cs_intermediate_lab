...
이력서는 사람이 자유롭게 쓴 **비정형 문서**입니다. 이것을 목록의 컬럼(지원자이름·직군·경력…)에 저장하려면 **정형 데이터로 변환**해야 합니다. 이 변환을 규칙 기반 파싱이 아니라 **AI 프롬프트**가 합니다 — "이 이력서에서 이름·이메일·직군·경력을 뽑고, 요약을 써줘"라고 시키는 것입니다.
...
지침 영역 하단에 **"이력서 문서를 제공해 주세요 : `Resume`"** 칩이 생성되어 있습니다. 이 칩을 클릭해 샘플 이력서 PDF 한 장을 선택합니다. 이후 상단 **`테스트`** 버튼을 눌러 모델 응답을 확인합니다.
...
{: .note }
테스트 결과가 JSON 형식으로 정상 출력되고, 이력서요약에 정량 수치가 보존되었는지 눈으로 확인합니다. 구조가 맞으면 프롬프트를 저장하고 흐름으로 돌아옵니다.

---

## ✅ 체크포인트

- [ ] **프롬프트 실행** 액션이 추가되어 있습니다.
- [ ] 새 사용자 지정 프롬프트가 만들어지고 지침이 입력되어 있습니다.
- [ ] 모델이 **GPT-4.1 mini**로 설정되어 있습니다.
